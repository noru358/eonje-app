...
function renderWinner(winner){
  ...
  document.body.dataset.datamode=data?.mode!=='live'?'demo':quality==='stale'?'stale':'live';
...
function renderTimeCards(winner){
  ...
  const labels=['먼저 가면','가장 좋은 한 시간','조금 늦게 가면'];const tags=['한적함 쪽','오늘의 추천','야경 쪽'];$('timeCards').innerHTML=[0,1,2].map(idx=>{const s=choices[idx]||winner.verdict?.best;const start=fmt(s?.time);let end='';if(s?.time){const d=new Date(s.time);d.setMinutes(d.getMinutes()+60);end=fmt(d.toISOString())}return `<article class="time-card ${idx===1?'featured':''}" data-index="${idx}"><div class="card-kicker">${labels[idx]}</div><strong>${start||'—'}${start?' – ':''}${end||''}</strong><p>${slotSummary(s)}</p><span class="tag">${tags[idx]}</span></article>`}).join('');
}
