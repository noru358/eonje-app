...
test('autumn is a physical park-specific source asset, not a tinted blossom fallback',()=>{for(const place of ['mangwon','yeouido','ichon','jamsil'])assert.match(sceneCss,new RegExp(`scenes/${place}/autumn/master-v2\\.png`));assert.match(sceneCss,/banpo\/autumn\/night-v2\.png/);assert.match(sceneCss,/ttukseom\/autumn\/night-v2\.png/);assert.match(assetFetch,/Wikimedia/);assert.match(assetFetch,/AUTUMN_SOURCES/);assert.match(assetFetch,/sharp/);assert.equal(pkg.dependencies.sharp,'0.34.3');});
